...
**Link target deep-link:** `link_type='package'` navigates `'/user/buy'` with `state:{ packageId, promoId }` — UserBuy reads `location.state?.packageId` to preselect and `promoId` to detect a discount. Don't drop the state or package promos stop deep-linking.

**Promo discounts (harga coret):** promos can carry an optional discount, but ONLY when `link_type='package'`: columns `discount_type` ('none'|'percent'|'amount') + `discount_value` on the promos table. Pricing is authoritative server-side via `resolvePurchaseAmount(pkg, promoId)` in server.ts — used by BOTH `/api/transactions` (saldo, reads `promo_id`) and `/api/payment/create-qris` (reads `promoId`). It re-fetches the promo and only discounts when active + in date window + link_type='package' + link_value===pkg.id. Client-sent `amount` is ignored. QRIS webhook stays consistent because it verifies the stored (already-discounted) transaction amount.
**Why:** price must never be trusted from client; UserBuy only shows/applies the discount when the carried promoId matches the selected package.
