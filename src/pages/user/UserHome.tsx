    ...
    <div className="space-y-5 pb-6 mt-2">
      ...
      <div className="px-5">
        <motion.div 
          ...
          className="rounded-[24px] p-5 bg-brand relative overflow-hidden shadow-md"
        >
          {/* Subtle decoration */}
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-[32px] font-bold text-white mb-5 tracking-tight">
              {formatRupiah(currentUser?.balance ?? 60000)}
            </h2>
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className="flex-1 flex justify-center items-center gap-1.5 bg-white text-brand font-semibold px-4 py-2 rounded-[14px] text-[13px] transition-transform hover:scale-[1.02] active:scale-[0.98]"
              >
      ...
      {/* active voucher area */}
      <div className="px-5">
        <div className="bg-surface border border-white/5 rounded-[20px] p-3.5 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 rounded-[12px] bg-success/10 flex items-center justify-center border border-success/20">
             ...
             <div>
                ...
                <p className="text-success font-mono text-[11px] font-bold mt-0.5">{activeTx?.voucher_code || 'MV7M-LQ66-G'}</p>
             </div>
          </div>
      ...
      <section className="px-5">
        <div className="flex justify-between items-center mb-3">
          ...
          <button onClick={() => navigate('/user/packages')} className="text-[13px] font-medium text-brand flex items-center">Lihat Semua <ChevronRight className="w-3.5 h-3.5 ml-0.5" /></button>
        </div>

        <div className="flex overflow-x-auto gap-3 hide-scrollbar snap-x pb-4">
          {packages.slice(0,3).map((pkg, idx) => (
            <div key={pkg.id} className="min-w-[140px] w-[140px] bg-surface border border-white/5 rounded-[20px] relative overflow-hidden snap-start shrink-0 shadow-sm">
               {/* top solid bar */}
               <div className="absolute top-0 left-0 w-full h-[3px] bg-brand" />
               <div className="p-3">
                   <div className="bg-brand/10 text-brand text-[9px] font-bold px-1.5 py-0.5 rounded w-fit mb-2">
                       {idx === 0 ? 'Murah' : idx === 1 ? 'Populer' : 'Terlaris'}
                   </div>
                   <h4 className="font-semibold text-white/90 text-[13px] leading-tight mb-1">{pkg.name}</h4>
                   <p className="text-[16px] font-bold text-white mb-3 tracking-tight">{formatRupiah(pkg.price)}</p>
