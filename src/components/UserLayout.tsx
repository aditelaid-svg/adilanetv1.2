      ...
      <div className="relative z-10 w-full max-w-md bg-[#000000] border-x border-white/5 min-h-screen flex flex-col relative overflow-x-hidden">
        ...
        <nav className="fixed bottom-0 w-full max-w-md mx-auto z-50">
          ...
          <div className="relative flex justify-around items-center h-[80px] px-2 pb-safe">
            ...
            <NavLink to="/user/" end className={({ isActive }) => cn("flex flex-col items-center justify-center w-16 h-full transition-colors relative group", isActive ? "text-brand" : "text-slate-500 hover:text-slate-300")}>
              {({ isActive }) => (
                <>
            ...
            <NavLink to="/user/packages" className={({ isActive }) => cn("flex flex-col items-center justify-center w-16 h-full transition-colors relative group mr-4", isActive ? "text-brand" : "text-slate-500 hover:text-slate-300")}>
              {({ isActive }) => (
                <>
            ...
            <div className="absolute left-1/2 -top-6 -translate-x-1/2 w-16 h-16 bg-[#000000] rounded-full flex items-center justify-center z-20">
              <button 
                ...
                className="w-14 h-14 rounded-full bg-brand shadow-[0_4px_14px_rgba(10,132,255,0.4)] transition-transform hover:scale-105 active:scale-95 flex items-center justify-center"
              >
                <ShoppingCart className="w-6 h-6 text-white" fill="currentColor" />
              </button>
            </div>

            <NavLink to="/user/history" className={({ isActive }) => cn("flex flex-col items-center justify-center w-16 h-full transition-colors relative group ml-4", isActive ? "text-brand" : "text-slate-500 hover:text-slate-300")}>
               {({ isActive }) => (
                <>
            ...
            <NavLink to="/user/profile" className={({ isActive }) => cn("flex flex-col items-center justify-center w-16 h-full transition-colors relative group", isActive ? "text-brand" : "text-slate-500 hover:text-slate-300")}>
               {({ isActive }) => (
                <>
